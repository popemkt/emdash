...
const TaskModal: React.FC<TaskModalProps> = ({
  ...
  const [initialPromptWorkflow, setInitialPromptWorkflow] = useState<WorkflowTemplate | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const trimmedPrompt = hasInitialPromptSupport ? initialPrompt.trim() : '';
    const requiresWorkflowSelection = Boolean(trimmedPrompt);
    if (requiresWorkflowSelection && !initialPromptWorkflow) {
      setWorkflowError('Select a workflow template when using an initial prompt.');
      return;
    }
    setWorkflowError(null);
    ...
    try {
      onCreateTask(
        ...
        trimmedPrompt ? (initialPromptWorkflow as WorkflowTemplate) : undefined,
        agentRuns,
        selectedLinearIssue,
        selectedGithubIssue,
        selectedJiraIssue,
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <TaskAdvancedSettings
            ...
            onInitialPromptChange={(value) => {
              setInitialPrompt(value);
              if (!value.trim()) {
                setWorkflowError(null);
              }
            }}
            initialPromptWorkflow={initialPromptWorkflow}
            onInitialPromptWorkflowChange={(value) => {
              setInitialPromptWorkflow(value);
              setWorkflowError(null);
            }}
          ...
          <DialogFooter>
            {workflowError ? (
              <p className="mr-auto text-xs text-destructive">{workflowError}</p>
            ) : null}
            <Button
              type="submit"
              disabled={
                !!validate(taskName) ||
                (hasInitialPromptSupport && !!initialPrompt.trim() && !initialPromptWorkflow)
              }
            >
              Create
            </Button>
